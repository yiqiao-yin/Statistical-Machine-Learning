...
lasso.VS <- function(
  ...
  train.x <- as.matrix(train[,-1]); dim(train.x)
  ...
  model <- cv.glmnet(
    ...
    family = fam)
  sum <- summary(model)
  selected.variable <- rownames(data.frame(coef(model, model$lambda.min)[,1]))[c(coef(model, model$lambda.min)[,1] != 0)]
